...
class PageManagerHelper {
  ...
  public function getVariantTitle($page_manager_page_variant = NULL, Request $request = NULL) {
    ...
    if ($page_manager_page_variant != NULL) {
      ...
      $variant_title = $this->getTitle($page_manager_page_variant->getTitle(), $page_manager_page_variant->getContexts($request));
    }
    ...
    foreach ($contexts as $context_name => $context) {
      ...
      if (preg_match('/entity:(\w+)/', $data_type, $matches)) {
        ...
        if (is_numeric($context->getContextValue())) {
          ...
          $context = $entity ? new Context($context->getContextDefinition(), $entity) : NULL;
        }
      }
      if ($context) {
